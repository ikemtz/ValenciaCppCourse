...
void Graph::Parse(const string& little_langague_data)
{ 
...
void Graph::ParseLabel(const string& little_langague_data) {
	_label = GetRegexMatches(regex("label ([\\w ,-]+)"), little_langague_data)[0];
	cout << "Label: " << _label << endl;
}

void Graph::ParseCoordinates(const string& little_langague_data) {
...
void Graph::ParseTicksAndRanges(const string& little_langague_data) {
	regex text{ "([a-z ]+)[a-z]" };
	regex numeric{ "[0-9]+" };
...
vector<string> Graph::GetRegexMatches(const regex lookup, string data, int groupMatchIndex) {
	return GetRegexMatches(lookup, data, vector<int> {groupMatchIndex});
}

vector<string> Graph::GetRegexMatches(const regex lookup, string data, vector<int> groupMatchIndexes) {
